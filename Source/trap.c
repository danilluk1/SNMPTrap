...
uint8_t version[] = { 0x02, 0x01, 0x01 }; //Version(0x02-Integer, 0x01 - Length, 0x01 - Value)//1
...
uint8_t value[] = { 0x02, 0x01, 0x01};//Value(0x02-Integer, 0x01 - Object ID, 0x01 - Length 0x01 - Value)
uint8_t sysUpTime[] = { 0x43, 0x02, 0x13, 0x3 };//Произвольный, статический
...
void sendSNMPTrap(uint8_t* out_buf, uint8_t* length) {
	...
	#pragma endregion
	uint8_t le = (uint8_t)(sizeof(packet_bytes) / sizeof(packet_bytes[0]));
	*length = le;
	memmove(out_buf, packet_bytes, sizeof(uint8_t) * le);
}
